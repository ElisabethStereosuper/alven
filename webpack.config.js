...
const config = (env, options) => {
    ...
    if (devMode) {
        rules.push({
            ...
            include: [
                ...
                path.resolve(__dirname, 'wp-content', 'themes', 'alven', 'src', 'js')
            ],
            options: {
                sourceMap: devMode
            }
    ...
    return {
        cache: !devMode,
        entry: './wp-content/themes/alven/src/js/main.js',
        output: {
            path: path.resolve(__dirname, 'wp-content/themes/alven/js'),
            filename: '[name].js',
            // Public path is important for dynamic imports. It'll help webpack to retrieve bundles by name and not by ids
            publicPath: '/wp-content/themes/alven/js/',
